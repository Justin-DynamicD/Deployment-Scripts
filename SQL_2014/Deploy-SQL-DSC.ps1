Configuration SQLInstall
{
    ...
    Node $AllNodes.where{ $_.Role.Contains("SqlServer") }.NodeName
    {
        ...
        File SQLServerIso
        {
            SourcePath = "$PackagePath\Sources\sql_server_2014_standard.iso"
        ...
        {
            SourcePath = "$PackagePath\ConfigurationScripts\SQL-Configuration.ini"
            DestinationPath = "c:\temp\ConfigurationFile.ini"
            Type = "File"
            Ensure = "Present"
        ...
        Script InstallSQLServer
        {
            ...
            SetScript =
            {
                ...
                $cmd = "$setupDriveLetter\Setup.exe /ConfigurationFile=c:\temp\ConfigurationFile.ini /SQLSVCPASSWORD=Qli1zahbTESkp8WNIE1w /AGTSVCPASSWORD=EcNnMmHbt9fjpdn1Ybwo /IAcceptSQLServerLicenseTerms"
                Write-Verbose "compiled install command: $cmd"
...
# command for RM
#SQLInstall -ConfigurationData $configData -PackagePath "\\FS-01\Deployment" -WinSources "d:\sources\sxs"
